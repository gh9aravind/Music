...
class MusicRepository(
    ...
    /**
     * Searches plain YouTube video uploads (not the official YT Music catalog)
     * for [query] - this is how movie/regional video-songs that never got
     * indexed as an official YT Music "song" get found. Playback still only
     * ever plays the audio track, same as [searchYouTube].
     */
    suspend fun searchYouTubeVideos(query: String): List<Track> = withContext(Dispatchers.IO) {
        try {
            com.example.data.youtube.YouTubeMusicService.searchVideos(query)
        } catch (e: Exception) {
            Log.e(TAG, "Failed searching YouTube videos: ${e.message}")
            emptyList()
        }
    }
